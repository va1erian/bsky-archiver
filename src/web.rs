...
async fn healthz(State(state): State<WebState>) -> Response {
    ...
    let status = if snapshot.any_error() {
...
async fn dashboard(State(state): State<WebState>) -> Result<Response, WebError> {
    ...
    // One row per configured feed, named by its display name, so the
    // dashboard shows each feed's health (Connected / Degraded / at-cap)
    // independently.
    let starting = crate::health::SubsystemHealth::degraded("starting up");
    for feed in &state.app.feeds {
        let entry = snapshot.feeds.get(&feed.slug).unwrap_or(&starting);
        health.push(templates::subsystem_row(
...
async fn fetch_excerpts(store: &ArchiveStore, items: &[PostSummary]) -> Vec<Option<String>> {
    let fetches = items.iter().map(|item| async move {
        match store.get_post(item.category.clone(), &item.at_uri).await {
...
fn build_category_options(
    selected: Option<&str>,
    feeds: &[crate::config::ResolvedFeed],
    ...
    // Each configured feed, by display name — the second level of the
    // posts/likes/bookmarks/feed selector. The query value is the feed's
    // `feeds/<slug>` category token, which `Category::from_str` round-trips.
    for feed in feeds {
        let value = Category::Feed(feed.slug.clone()).as_dir().into_owned();
        options.push(templates::CategoryOption {
...
async fn post_detail(
    ...
    let at_uri = id;
    // Discover which categories this item is archived under from the index
    // (an item can be in several), so a feed post — including one under a
    // feed no longer in `BSKY_WATCH_FEEDS` — is found without knowing its
    // category up front. Fall back to the three built-ins if the index has
    // no row (e.g. an out-of-band write).
    let mut categories = state.app.store.find_categories(&at_uri).await?;
    if categories.is_empty() {
        categories = vec![Category::Post, Category::Like, Category::Bookmark];
    }
    for category in categories {
        if let Some(record) = state.app.store.get_post(category.clone(), &at_uri).await? {
...
/// The query token the gallery uses for a category in its own links
/// (`/gallery?category=like`, `/gallery/export?category=like`): the singular
/// form for the three built-ins, `feeds/<slug>` for a feed (URL-encoded by
/// the caller when placed in an href).
fn category_token(category: &Category) -> String {
    match category {
        Category::Post => "post".to_string(),
        Category::Like => "like".to_string(),
        Category::Bookmark => "bookmark".to_string(),
        Category::Feed(_) => category.as_dir().into_owned(),
    }
}

/// A filesystem-safe label for a category, used in the export zip's
/// filename (which must not contain a `/`).
fn category_filename_label(category: &Category) -> String {
    match category {
        Category::Post => "posts".to_string(),
        Category::Like => "likes".to_string(),
        Category::Bookmark => "bookmarks".to_string(),
        Category::Feed(slug) => format!("feed-{slug}"),
    }
}
...
async fn gallery(
    ...
    let result = state
        .app
        .store
        .list_media(category.clone(), page, page_size)
        .await?;
    let items: Vec<_> = result.items.iter().map(templates::gallery_item).collect();

    let cat_token = category.as_ref().map(|c| {
        percent_encoding::utf8_percent_encode(
            &category_token(c),
            percent_encoding::NON_ALPHANUMERIC,
        )
...
async fn gallery_export(
    ...
    let category = parse_gallery_category(query.category.as_deref())?;
    let items = state.app.store.list_export_media(category.clone()).await?;
...
async fn stream_zip(
    ...
    for item in items {
        let file = match store
            .open_media(item.category.clone(), &item.post_at_uri, &item.filename)
            .await
        {
            Ok(Some(file)) => file,
            Ok(None) => {
...
async fn config_view(State(state): State<WebState>) -> Result<Response, WebError> {
    let config = &state.app.config;
    let redacted = "<redacted>".to_string();
    let watch_feeds_value = if config.watch_feeds.is_empty() {
        "(none)".to_string()
    } else {
        config
            .watch_feeds
    ...
    let rows = vec![
        ...
        templates::ConfigRow {
            key: "BSKY_WATCH_FEEDS",
            value: watch_feeds_value,
            redacted: false,
        },
        templates::ConfigRow {
            key: "ARCHIVE_DIR",
        ...
        templates::ConfigRow {
            key: "FEED_MAX_BYTES",
            value: config.feed_max_bytes.to_string(),
            redacted: false,
        },
    ];

    // Per-feed detail: resolved URI, display name, cap, and current bytes
    // used (queried live from the index).
    let mut feeds = Vec::with_capacity(state.app.feeds.len());
    for feed in &state.app.feeds {
        let used = state
            .app
            .store
            .category_media_bytes(&Category::Feed(feed.slug.clone()))
            .await?;
        feeds.push(templates::ConfigFeedRow {
            name: feed.label().to_string(),
            input: feed.input.clone(),
            uri: feed.at_uri.clone(),
            cap: templates::format_bytes(config.feed_max_bytes),
            used: templates::format_bytes(used),
        });
    }

    Ok(askama_axum::into_response(&templates::ConfigTemplate {
        version: templates::APP_VERSION,
        rows,
        feeds,
    }))
