...
pub(super) async fn dashboard(State(state): State<WebState>) -> Result<Response, WebError> {
    ...
    let tumblr_likes_count = store
        ...
        .await?
        .total_items;
    let pixiv_bookmarks_count = store
        .list_posts(Some(Category::PixivBookmark), 1, 1)
        .await?
        .total_items;
    ...
    let health = vec![
        ...
        templates::subsystem_row("Pixiv bookmarks", &snapshot.pixiv_bookmarks),
