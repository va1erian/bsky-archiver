...
5. Optionally, the liked posts of a configured Tumblr account (see the
   ...
   official API.
6. Optionally, the bookmarked illustrations of a configured Pixiv account
   (see the `PIXIV_*` environment variables below) — poll-only, via
   Pixiv's unofficial App API.
...
| `pixiv` | The Pixiv App API client (unofficial, what Pixiv's own Android app speaks; OAuth2 refresh-token auth refreshed automatically) and the Pixiv bookmarks poller, which archives the configured Pixiv account's bookmarked illustrations into the `pixiv_bookmarks` category on the same adaptive polling cadence. Image downloads set the `Referer` Pixiv's CDN requires. Only runs when `PIXIV_REFRESH_TOKEN` is configured. |
...
| `PIXIV_REFRESH_TOKEN` | *(unset)* | Pixiv App API refresh token enabling the Pixiv bookmarks archiver. Unset disables Pixiv archiving. The App API is unofficial (it is what Pixiv's own Android app speaks), so endpoints can change without notice. Get a refresh token once via the PKCE login flow (e.g. [ZipFile's pixiv_auth.py](https://gist.github.com/ZipFile/c9ebedb224406f4f11845ab700124362) or `gppt`); the access token it exchanges for expires ~hourly and is refreshed automatically. |
| `PIXIV_POLL_INTERVAL_SECONDS` | `300` | Baseline interval for Pixiv bookmarks polling (only used when `PIXIV_REFRESH_TOKEN` is set). The unofficial App API has no documented rate limit but tolerates only ~1 request/second; page fetches within a backfill walk are paced 2s apart. |
